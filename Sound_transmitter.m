...
aa = 0;
x1= '1314+5354/1588+1561/1561+4568/1565+8168/1654+5164';
%x1=[x1 '/' x1 '/' x1 '/' x1 '/' x1];
disp('开始编码……');   
 %---------------------------------------------------------------------
bbbb1 = yima(x1);      %-----------------译码，并加巴克码     
c1 = tuokuan(bbbb1,X)   %-----------------%拓宽基带信号
modulatedData1=zaibo(f1,f2,fs,c1);%-----载波 f1，f2，fs,拓宽的基带信号
DOWN=160;%下采样
%------------------构建滤波器
%------------------先映射字符0~9和+-*/，然后通过汉明编码，一定程度减少误码率
 %---------------------------------------------------------------------
sound(modulatedData1,fs);%----发射声音
%---------------------------------------------------------------------
%-----全部都在画图,没什么用；
bittosend_temp1=bbbb1;
bittosend_temp1=int2str(bittosend_temp1);
disp(bittosend_temp1);
disp(length(modulatedData1));
disp(length(bittosend_temp1));
disp(length(modulatedData1)/length(bittosend_temp1));  
%---------------------------------------------------------接收
%end
pause(150);
% 
...
function [F] = yima(LCD_str)
        ...
        %增加前面的保护码，同步吗巴克码
        F=[ones(160,1);[1 1 1 1 1 0 0 1 1 0 1 0 1]';b2;zeros(11,1);[1 1 1 1 1 0 0 1 1 0 1 0 1]';];
  end 
